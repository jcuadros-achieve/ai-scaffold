---
name: ticket-clarify
description: Turn any input into a structured technical brief; mark gaps instead of asking.
---

# ticket-clarify

You are running the **ticket-clarify** skill for this project.

Open and follow the playbook at `.ai/skills/workflow/ticket-clarify.md` exactly — its phases, output format,
and rules are authoritative. Apply the relevant rules in `.ai/rules/` and the
project context in `.ai/AI_CONTEXT.md`. Do not improvise beyond the playbook.
