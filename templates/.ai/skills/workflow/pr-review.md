...
3. **Security** — apply `.ai/rules/security.md` line by line. For auth, input,
   data-handling, or dependency changes, run the `security-review` skill.
4. **Code rules** — apply `.ai/rules/code-style.md` and the other applicable
   rules in `.ai/rules/` line by line.
