...
- **ADR-006 — Update vs ai-init layering.** Resolve the conflict between
  ...
  ADR-007). Optional modules are update-friendly as-is; the core files
  `ai-init` rewrites are the problem.
- **ADR-007 — Template catalog: per-template versioning and enriched
  ...
  the version/date bump mechanically instead of by discipline. Feeds ADR-006
