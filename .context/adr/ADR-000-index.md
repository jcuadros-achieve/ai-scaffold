...
| [ADR-017](ADR-017-update-command-seed-files.md) | `update` as its own command — seed files installed once, a changelog for the rest | 2026-06-15 | Accepted | Per-template `track: seed \| reconcile` in the catalog: `seed` files (`CLAUDE.md`, rules, `ai-init`-rewritten skills) install once and are never reconciled; `update` becomes a wizard-free command separate from `install` and surfaces a catalog-derived changelog. Supersedes ADR-006 for seed files only. |

² Decision 1 (keeping the Copilot pointer) superseded by ADR-011.

¹ Decision 2's ADR-drafting timing superseded by ADR-004.

³ Superseded by ADR-017 for *seed* files only (they leave the three-way
classification); the three-way path stands for `reconcile` files.
