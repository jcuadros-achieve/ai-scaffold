...
| [ADR-022](ADR-022-cleanup-scaffold-artifacts.md) | Clean up ephemeral `.scaffold/` artifacts when ai-init completes | 2026-06-22 | Accepted | ai-init removes `.scaffold/` as the final step of Phase 5, after the payload is written; no later CLI command reads it (update/status/diff key off `.claude/.scaffold-state.json`). Cleanup is the agent's (it owns the ephemeral workspace, not payload); ADR-017 §4's re-run-without-re-scan benefit is preserved *during* the flow, lost only *after* completion. Refines ADR-017 §4. |
